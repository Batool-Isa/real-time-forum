...
func InsertCategories(category_name string) error {
	...
	if err != nil {
			log.Println("lkjgelgkdjgvlkgesnvblkdnhblknglknclk66666666666666")
...
func InsertPost(user_id int, post_heading string, post_data string, categoryName []string) error {
	...
	stmt, err := db.Prepare("INSERT INTO Post(user_id, post_heading, post_data) VALUES (?, ?, ?)")
	if err != nil {
		log.Println(err)
		return err
	}
...
func InsertComment(comment string, user_id int, postId int) error {
	...
	stmt, err := db.Prepare("INSERT INTO Comment(comment, user_id, post_id) values (?, ?, ?)")
	if err != nil {
		log.Println(err)
		return err
	}
...
func InsertCommentLikes(comment_id int, user_id int) error {
	...
	var count int
	err := db.QueryRow("SELECT COUNT(*) FROM Comment_Like WHERE comment_id = ? AND user_id = ?", comment_id, user_id).Scan(&count)
	if err != nil {
		log.Println(err)
		return err
	}
	...
	if count == 1 {
		// If the like exists, delete it (unlike)
		stmt, err := db.Prepare("DELETE FROM Comment_Like WHERE comment_id = ? AND user_id = ?")
		if err != nil {
			log.Println(err)
			return err
		}
...
func InsertCommentDislikes(comment_id int, user_id int) error {
	...
	var count int
	err := db.QueryRow("SELECT COUNT(*) FROM Comment_Dislike WHERE comment_id = ? AND user_id = ?", comment_id, user_id).Scan(&count)
	if err != nil {
		log.Println(err)
		return err
	}
	...
	if count == 1 {
		// If the like exists, delete it (unlike)
		stmt, err := db.Prepare("DELETE FROM Comment_Dislike WHERE comment_id = ? AND user_id = ?")
		if err != nil {
			log.Println(err)
			return err
		}
		...
		// If the like does not exist, insert it (like)
		stmt, err := db.Prepare("INSERT INTO Comment_Dislike(comment_id, user_id) values (?, ?)")
		if err != nil {
			log.Println(err)
			return err
		}
...
func InsertSession(session string, user_id int) error {
	...
	stmt, err := db.Prepare("INSERT INTO Session(session, user_id, timestamp) values (?, ?, ?)")
	if err != nil {
		log.Println(err)
		return err
	}
